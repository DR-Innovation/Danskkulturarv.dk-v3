  ...
  <div class="col-sm-7 col-md-8 col-xs-9">
    <div class="input-group">
      ...
      <span class="input-group-btn">
        <button type="submit" class="btn btn-default btn-search btn-lg" id="searchsubmit"><i class="icon-search icon-large"></i>
        </button>
      </span>
    </div>
...
<?php foreach (WPDKAObject::$format_types as $format_type => $args) : if ($format_type == WPDKAObject::TYPE_IMAGE_AUDIO || $format_type == WPDKAObject::TYPE_UNKNOWN) {
     continue;
 }
...
<?php endforeach; ?>
  ...
  <div class="col-sm-2 col-xs-3 btn-advanced-search-container">
    <button class="btn btn-default btn-lg btn-advanced-search collapsed dropdown-toggle btn-block <?php echo $advanced_search_expanded; ?>" type="button" data-toggle="collapse" data-target="#advanced-search-container">
      <span class="hidden-xs"><?php _e('Options', 'dka'); ?></span> <i class="icon-caret-down icon-large"></i>
    </button>
  </div>
  <div id="advanced-search-container" class="col-xs-12 collapse<?php echo $advanced_search_expanded; ?>">
    ...
    <div class="col-xs-12 filter-container filter-media-type visible-xs">
      ...
      <label class="btn btn-default filter-btn filter-btn-all"><?php _e('All Formats', 'dka'); ?></label>
      <?php foreach (WPDKAObject::$format_types as $format_type => $args) : if ($format_type == WPDKAObject::TYPE_IMAGE_AUDIO || $format_type == WPDKAObject::TYPE_UNKNOWN) {continue;} ?>
        <label title="<?php echo $args['title']; ?>" for="<?php echo WPDKASearch::QUERY_KEY_TYPE.'-'.$format_type; ?>" class="btn btn-default filter-btn filter-btn-single">
          ...
          <!-- <b> <?php echo $args['title']; ?></b> -->
           <?php echo get_facet_count(WPDKASearch::QUERY_KEY_TYPE, $args['chaos-value']) ?>
        </label>
      <?php endforeach; ?>
    </div>
    ...
    <div class="col-xs-12 col-sm-12 filter-container filter-organizations">
      ...
      <label class="btn btn-default filter-btn filter-btn-all"><?php _e('All Organizations', 'dka'); ?></label>
      ...
      $count = get_facet_count(WPDKASearch::QUERY_KEY_ORGANIZATION, $organization['chaos_titles']);?>
        <label for="<?php echo WPDKASearch::QUERY_KEY_ORGANIZATION.'-'.$organization['slug']; ?>" class="btn btn-default filter-btn filter-btn-single">
          ...
          <b><?php echo $organization['title']; ?></b> <?php echo $count ?>
        </label>
      <?php endforeach; ?>
