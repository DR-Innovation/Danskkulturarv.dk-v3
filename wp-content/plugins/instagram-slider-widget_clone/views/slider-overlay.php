...
<div class="pllexislider overlay">
		...
		<?php 
			if ( isset($data_arr) && is_array($data_arr) ) {
				foreach ($data_arr as $data) {
					...
					if ( $created_time ) {
						echo '<span class="jr-insta-time">'. human_time_diff( $created_time ) . ' siden</span>' . "\n";
					}
					echo '<span class="jr-insta-username"><a target="_blank" href="'. $user_url .'">'. $user_name .'</a></span>' . "\n";
					if ($text) {
    ...
    <div class="instagram_logo"><img src="<?php echo plugins_url( '../instagram_logo.png' , __FILE__ ); ?>" alt="Instagram" /></div>
</div>
